...
class EndToEndModel(BertPreTrainedModel):
...
class trainModel():
    ...
    def get_result_socre(self, answers, labels, predict_type, answer_start, answer_end, input_tokens):
        ...
        for i in range(len(answers)):
            if predict_type[i] != 3:
                predict_answer = tokenizer.convert_tokens_to_string(input_tokens[i][answer_start[i]:answer_end[i]+1])
...
def main(arg):
    ...
    f = open('./data/finalCleanQuestionGeneration/bridge/postfix_three_with_other_data_end_to_end.pkl', 'rb')
    ...
    input_tokens = [f[7] for f in features][int(length*arg.part):]
