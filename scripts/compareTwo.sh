...
for number in 2;do
    ...
    for((i=0;i<${#seed[*]};i++)); do
        ...
        python compareTwo/domainPretrain.py --epochs=${number} --cuda=${cuda} --seed=${seed[i]}
        cmd="python finetune/trainAndEval.py --pre=True --model_path=./model/chinese_macbert_base --cuda=${cuda} --train_data=${domain}"
        tempi=`expr ${i} + 1`
