...
#     number=2
#     python idea/domainPretrainNotLock.py --epochs=${number} --cuda=${cuda} --seed=${se}
...
#             echo ${ncmd}
#             ${ncmd} &>./log/${domain}/middleNotLockFinalLock2/${se}/log${tempi}/chinese_bert_wwm_${dz}.log
#         done
#         ncmd="${cmd} --seed=${seed[i]} --full_data=True"
#         echo ${ncmd}
#         ${ncmd} &>./log/${domain}/middleNotLockFinalLock2/${se}/logfull/chinese_bert_wwm_${tempi}.log
#     done
#     echo ""
# done
#! /bin/bash
...
domain=bridge
cuda=${1}
logname=middleNotLockFinalLock_reasonable
...
for number in 100;do
    ...
    for((i=0;i<${#seed[*]};i++)); do
        ...
        python idea/domainPretrainNotLock.py --epochs=${number} --cuda=${cuda} --seed=${seed[i]}
        cmd="python finetune/trainAndEval.py --pre=True --lock=True --cuda=${cuda} --train_data=${domain}"
        tempi=`expr ${i} + 1`
        for dz in 16 32 64 128 256 512 1024 2048; do
