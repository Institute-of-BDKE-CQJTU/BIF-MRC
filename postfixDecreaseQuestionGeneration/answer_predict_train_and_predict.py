...
def main(arg):
    ...
    f = open('./data/postfixDecreaseQuestionGeneration/bridge/answer_predict_train.pkl', 'rb')
    ...
    f.close()
    f = open('./data/postfixDecreaseQuestionGeneration/bridge/answer_predict_test.pkl', 'rb')
    features = pickle.load(f)
    # eval
    all_input_ids = torch.tensor([f[0] for f in features], dtype=torch.long)
    all_input_mask = torch.tensor([f[1] for f in features], dtype=torch.long)
    ...
    f.close()
    file=open('./postfixDecreaseQuestionGeneration/predict_start_index.txt','w')  
    file.write(str(predict_start_index))
    file.close()

    file=open('./postfixDecreaseQuestionGeneration/predict_end_index.txt','w')  
...
def tokenizer_test_data():
    ...
    with open('./data/postfixDecreaseQuestionGeneration/bridge/answer_predict_test.pkl', 'wb') as f:
