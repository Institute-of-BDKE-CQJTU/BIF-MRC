...
def tokenize_question_type():
    ...
    for i in range(count):
        if int(datas[i]['postfix_index']) != 6:
    ...
    with open('./data/newQuestionGeneration/bridge/question_type_train.pkl', 'wb') as f:
...
def tokenizer_answer_predict():
    ...
    with open('./data/newQuestionGeneration/bridge/answer_predict_train.pkl', 'wb') as f:
        pickle.dump(features, f)
...
def tokenizer_end_to_end():
    ...
    for i in range(count):
        ...
        if postfix_index not in [0, 4, 5]:
            continue
        input = tokenizer(context, padding='max_length', truncation=True, max_length=512, return_tensors='pt')
        input_tokens = tokenizer.tokenize(context)
        answer = datas[i]['answer']
        str_start = datas[i]['answer_start']
        answerTokenIds = tokenizer(answer, return_tensors='pt')['input_ids'][0].tolist()[1:-1]
        try:
            answerStart, answerEnd = findStartEnd(input['input_ids'][0].tolist(), answerTokenIds, str_start)
            postfix_index = datas[i]['postfix_index']
        except:
            print(count, i, answerTokenIds)
            continue
        ...
        features.append([input_ids, attention_mask, token_type_ids, temp_dict[postfix_index], answerStart, answerEnd, answer, input_tokens])
...
def tokenizer_pretuning_train_data():
    ...
    for i in range(count):
        ...
        try:
            ...
            temp_index.append(i)
            continue
        # if question_type_number[postfix_to_index[postfix]] < 759:
    ...
    with open('./data/newQuestionGeneration/bridge/pretuning_data_train_new4.pkl', 'wb') as f:
        pickle.dump(features, f)
