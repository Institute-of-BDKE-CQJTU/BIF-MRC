...
def main(arg):
    ...
    f = open('./data/newQuestionGeneration/bridge/question_type_train.pkl', 'rb')
    ...
    f = open('./data/newQuestionGeneration/bridge/question_type_test.pkl', 'rb')
    features = pickle.load(f)
    # train
    length = len(features)
    all_input_ids = torch.tensor([f[0] for f in features], dtype=torch.long)
    ...
    f.close()
    file=open('./newQuestionGeneration/question_postfix.txt','w')  
...
def tokenizer_test_data():
    ...
    with open('./data/newQuestionGeneration/bridge/question_type_test.pkl', 'wb') as f:
