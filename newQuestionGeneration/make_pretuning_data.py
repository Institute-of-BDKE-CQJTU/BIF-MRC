...
file=open('./newQuestionGeneration/predict_start_index.txt','r')
predict_start_index_str = file.readlines()[0].strip()[1:-1]
predict_start_index = list(map(int, predict_start_index_str.split(',')))
file.close()

file=open('./newQuestionGeneration/predict_end_index.txt','r')
...
with open('./data/newQuestionGeneration/bridge/pretuning_data_train.json', 'w', encoding='UTF-8') as f:
